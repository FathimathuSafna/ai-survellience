...
const Managers = ({ data }) => {
  ...
  const itemsPerPage = 6; // Grid looks best with 6 items (2 rows of 3)
  ...
  // 2. Reset to Page 1 when searching (Crucial for UX)
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm]);
  ...
  const handleView = (manager) => { setSelectedManager(manager); setShowViewModal(true); };
  const handleEdit = (manager) => { setSelectedManager(manager); setShowEditModal(true); };
  const handleClose = () => { setShowViewModal(false); setShowEditModal(false); setSelectedManager(null); };
  ...
  return (
    <div className="managers-wrapper">
      <style>{`
        ...
        .modal-title { font-weight: 700; color: var(--text-dark); display: flex; align-items: center; gap: 10px; }
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Oversee leadership performance, team allocation, and store assignments.</p>
        </div>
        <button className="btn btn-solid" style={{ width: 'auto', padding: '10px 20px' }} onClick={() => { setSelectedManager({name:'', store:'', team:'', performance:0}); setShowEditModal(true); }}>
          + Add Manager
        </button>
      </div>
          ...
          currentManagers.map(manager => (
            <div key={manager.id} className="manager-card">
              ...
              <div className="card-actions">
                <button className="btn btn-outline" onClick={() => handleView(manager)}>View Details</button>
                <button className="btn btn-solid" onClick={() => handleEdit(manager)}>Edit</button>
              </div>
            </div>
          ))
        ) : (
        ...
        <div className="modal-overlay" onClick={handleClose}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title"><FaUserTie style={{ color: '#165d3c' }}/> Manager Profile</span>
            ...
            <div className="modal-body">
              <div style={{ textAlign: 'center', marginBottom: '24px' }}>
                ...
                <h2 style={{ margin: '0 0 4px', color: '#1e293b' }}>{selectedManager.name}</h2>
                <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', marginTop: '12px' }}>
                  <button className="btn btn-outline" style={{ padding: '8px' }}><FaEnvelope /></button>
                  <button className="btn btn-outline" style={{ padding: '8px' }}><FaPhone /></button>
                  <button className="btn btn-outline" style={{ padding: '8px' }}><FaLinkedin /></button>
                </div>
              </div>

              <div className="detail-row">
                <span className="detail-label"><FaMapMarkerAlt /> Assigned Store</span>
                <span className="detail-val">{selectedManager.store}</span>
              </div>
              <div className="detail-row">
                <span className="detail-label"><FaUsers /> Team Size</span>
                <span className="detail-val">{selectedManager.team} Employees</span>
              </div>
              <div className="detail-row">
                <span className="detail-label"><FaClock /> Years Experience</span>
                <span className="detail-val">{selectedManager.experience}</span>
              </div>
              <div className="detail-row" style={{ border: 'none' }}>
                <span className="detail-label"><FaChartLine /> KPI Score</span>
                <span className="detail-val" style={{ color: '#165d3c' }}>{selectedManager.performance}/100</span>
              </div>
            </div>
      ...
      {showEditModal && selectedManager && (
        <div className="modal-overlay" onClick={handleClose}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title"><FaEdit style={{ color: '#165d3c' }}/> Edit Manager</span>
              <button className="close-btn" onClick={handleClose}><FaTimes /></button>
            </div>
            <div className="modal-body">
              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', marginBottom: '6px' }}>Full Name</label>
                <input type="text" className="search-input" defaultValue={selectedManager.name} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', marginBottom: '6px' }}>Store</label>
                  <select className="search-input" defaultValue={selectedManager.store}>
                     {data?.managers?.map(m => <option key={m.id}>{m.store}</option>)}
                  </select>
                </div>
                <div>
                   <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', marginBottom: '6px' }}>Team Size</label>
                   <input type="number" className="search-input" defaultValue={selectedManager.team} />
                </div>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', marginBottom: '6px' }}>Performance Score (0-100)</label>
                <input type="number" className="search-input" defaultValue={selectedManager.performance} />
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn btn-outline" onClick={handleClose}>Cancel</button>
              <button className="btn btn-solid" onClick={handleClose}>Save Changes</button>
            </div>
          </div>
        </div>
      )}
